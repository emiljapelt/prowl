        ...
        return new List<MethodConstruct> {
            ...
            new MethodConstruct {
                ...
                method = async args => {
                    ...
                    var tocheck = pwl.products.Where(p => p.name.ToLower().StartsWith(filter));
                    if (tocheck.Count() == 0) return Failure("No products to be checked");

                    var tasks = new List<Task<(string text, ConsoleColor color)>>();

                    foreach(Product p in tocheck) tasks.Add(p.CheckProduct());
